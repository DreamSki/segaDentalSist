...
public class CreateReportOfRequests implements DatabaseCommand {
	...
	String baseQuery = "SELECT C.IDENTITY_CARD, C.LAST_NAME, C.FIRST_NAME," +
			...
			" CA.CITY, CA.STATE, CCT.NAME, PP.VOUCHER, PP.AMOUNT, CCC.BANK, CP.STATUS_ID, (SELECT COUNT(CB.ID)" +
			" FROM CLIENT_BENEFICIARY CB WHERE CB.CLIENT_PRODUCT_ID = CP.ID) AS BENEFICIARY " +
			...
			" WHERE C.ID = CP.CLIENT_ID AND CP.PRODUCT_ID = P.ID AND CP.CHECKER_ID = U.ID AND U.ROOM_ID = UR.ID AND C.ID = CA.CLIENT_ID AND" +
	...
	public Object executeDatabaseOperation(Connection conn) throws SQLException {
		...
		while(rs.next()) {
			...
			reportItem.setNumBenef(rs.getInt(15));
			list.add(reportItem);
		}
